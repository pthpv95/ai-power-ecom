...
- Sets `EXPIRE stream:{conversation_id} 3600` after done event — auto-cleanup
- Uses `_job_id=conversation_id` to prevent duplicate processing on double-submit
- `max_tries=2` for transient failure recovery

### SSE Endpoint (`GET /events/{conversation_id}`)
...
- Step 1: `POST /api/chat/stream` → get `{ conversation_id, status: "queued" }`
- Step 2: `fetch(/api/chat/events/${convId})` → read SSE via ReadableStream
...
4. **Arq's `_job_id` prevents double-processing** — if the frontend double-submits (e.g., user clicks Send twice), the second enqueue is silently deduplicated because the job ID matches.
